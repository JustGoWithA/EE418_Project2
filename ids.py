...
class IDS:
    ...
    def estimate_offset(self, a):
        ...
        if self.mode == 'state-of-the-art':
            ...
            # find current average offset
            sum = 0;
            # iterate over entire batch of N messages, starting at i=2
            for i in range(2, self.N+1):
                sum +=  a[i] - (a[0] + (i-1)*prev_mu_T_sec);
            curr_avg_offset_us = 1.0 * sum / (1.0*(self.N -1));
            
            
            # find accumulated offset
            curr_acc_offset_us = prev_acc_offset_us + math.abs(curr_avg_offset_us);
            ...
            # Find average offset: avg. offset = T - (a[N] - a[0]) /  N
            curr_avg_offset_us = self.T_sec - (a[self.N] - a[0]) / (1.0*self.N);
